    ...
    moving_icon(new QGraphicsPixmapItem)
{
    ...
    QPixmap frame_temp(":/resources/img/distanceBar.png");
    distance_frame->setPixmap(frame_temp.scaled(550,100));
    scene->addItem(distance_frame);
    distance_frame->setPos(0,0);

    QPixmap icon_temp(":/resources/img/avatar_stand.png");
    moving_icon->setPixmap(icon_temp.scaled(20,100));
    scene->addItem(moving_icon);
    moving_icon->setPos(0,0);

    //updates the tiemr that times out of the levels
