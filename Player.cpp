...
void Player::keyPressEvent(QKeyEvent *event)
{
    if(event->key()==Qt::Key_B && can_bomb_be_launched && bomb_bool_active){ //creates a bomb if the key B has been pressed
                // create a bullet
                QPixmap temp(":/resources/img/waterballoon.png");
                QPixmap bomb_pixmap (temp.scaled(75,75));

                Bomb* bomb = new Bomb(10,10,true);
                bomb->setPixmap(bomb_pixmap);
                bomb->setPos(x()+100,y()+50);
                scene()->addItem(bomb);
    ...
    else if(event->key()==Qt::Key_Space && can_bullet_be_launched ){ //creates a bullet if the space bar has been pressed
                // create a bullet
    ...
    else if (event->key()==Qt::Key_V && can_go_invincible)//if the key V has been pressed invincibility is created
    {
        QPixmap temp(":/resources/img/player_invincible.png");
...
void Player::timerEvent(QTimerEvent *)
{
    ...
    } else if(keys[Qt::Key_Right]){ //checks if the right key has been clicked
        if (pos().x() + 100 < 1200) { // only allows movement within screen boundary
            setPos(x()+10,y());
        }
    } else if(keys[Qt::Key_Up]){ //checks if the up button has been clicked
        if (pos().y() > 0) { // only allows movement within screen boundary
            setPos(x(), y()-10);
        }
    } else if(keys[Qt::Key_Down]){ //checks if the down key has been clicked
        if (pos().y() + 100 < 800) { // only allows movement within screen boundary
            setPos(x(), y()+10);
        }
    }
}
...
void Player::set_bomb_bool_active(bool i)
{
    ...
    {
        QPixmap temp2(":/resources/img/waterballoon.png");
        bombicon = new QGraphicsPixmapItem(temp2.scaled(80,80));
...
void Player::end_invincibility()
{
    ...
    QPixmap player_pixmap = temp.scaled(130,130);
